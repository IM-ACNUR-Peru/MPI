...
:::{.lead}
This document present an analysis of multidimensional poverty in Peru
:::
...
df_household$electricity <- NA
df_household[df_household$P108_3 > 1,"electricity"]<- 0 
df_household$electricity[is.na(df_household$electricity)] <- 1
#df_household$electricity <- as.numeric(df_household$electricity)
...
## INDICADOR x:The household has inadequate housing materials in any of the three components: floor, roof, or walls.
df_household <- df_household |>
          dplyr::mutate(housing = dplyr::case_when(
            (ind04_01 ==0 | ind04_02 ==0 | ind04_03 ==0) ~ 1,   
            TRUE ~ 0   ) )
df_household$housing  <- labelled::labelled(df_household$housing,
  labels = c( 'Hay necesidad' = 0, 'No hay necesidad/vulnerabilidad' =1 ),
  label = "INDICADOR x:The household has inadequate housing materials in any of the three components: floor, roof, or walls.")


deprivation_profile$housing <- df_household |>
  define_deprivation(
    .indicator = housing,
    .cutoff = ind04_01 ==0 | ind04_02 ==0 | ind04_03 ==0
  )
...
df_household$cooking_fuel <- NA
df_household[df_household$P109 > 3, "cooking_fuel"] <- 0                             
df_household$cooking_fuel[is.na(df_household$cooking_fuel)] <- 1
#df_household$cooking_fuel <- as.numeric(df_household$cooking_fuel)
...
df_household$assets <- NA
df_household[df_household$P110 <=2, "assets"] <- 0                            
df_household$assets[is.na(df_household$assets)] <- 1
#df_household$assets <- as.numeric(df_household$assets)   
...
df_household$assetsv2 <- NA
df_household[df_household$P110 <=3, "assetsv2"] <- 0                          
df_household$assetsv2[is.na(df_household$assetsv2)] <- 1
# df_household$assetsv2 <- as.numeric(df_household$assetsv2)    
...
```{r df_household_roster, echo=FALSE, cache = TRUE}
...
df_household_roster$nutrition <- NA
df_household_roster$nutrition[df_household_roster$P401_5 == 1] <- 1
df_household_roster$nutrition[is.na(df_household_roster$nutrition)] <- 0
#df_household_roster$nutrition <- as.numeric(df_household_roster$nutrition)
df_household_roster$nutrition  <- labelled::labelled(df_household_roster$nutrition,
  labels = c( 'Hay necesidad' = 0, 'No hay necesidad/vulnerabilidad' =1 ),
  label = "Afiliación a seguro de salud")
...
df_household_roster$child_mortality <- NA
df_household_roster$child_mortality[df_household_roster$P405_7 == 0 & df_household_roster$P406==1 ] <- 1
df_household_roster$child_mortality[is.na(df_household_roster$child_mortality)] <- 0
#df_household_roster$child_mortality <- as.numeric(df_household_roster$child_mortality)
...
df_household_roster<- df_household_roster  |>
  group_by(uuid)  |>
  mutate(nutrition_vf = sum_nutrition / n())  |>
  mutate(child_mortality_vf = sum_child_mortality / n())
...
df_household_roster$year_schooling <- NA
df_household_roster$year_schooling[df_household_roster$P205_A > 17 & (df_household_roster$P501 < 6 | df_household_roster$P501B < 6)] <- 1
df_household_roster$year_schooling[is.na(df_household_roster$year_schooling)] <- 0
#df_household_roster$year_schooling <- as.numeric(df_household_roster$year_schooling)
df_household_roster$year_schooling  <- labelled::labelled(df_household_roster$year_schooling,
  labels = c( 'Hay necesidad' = 0, 'No hay necesidad/vulnerabilidad' =1 ),
  label = "Personas de 18 a más años de edad tienen último nivel de estudios Educación Media Diversificada Completa/Secundaria completa")   

deprivation_profile$year_schooling <- df_household_roster |>
  define_deprivation(
    .indicator = year_schooling,
    .cutoff = P501 < 6 | P501B < 6,
    .collapse = TRUE
  )

##### INDICADOR 2: Asistencia a la escuela P205 < 18 & no asiste

df_household_roster$school_attendance <- NA
df_household_roster$school_attendance[(df_household_roster$P205_A < 18 & df_household_roster$P205_A > 2) & df_household_roster$P506 != 1] <- 1 #hay necesdidad
df_household_roster$school_attendance[is.na(df_household_roster$school_attendance)] <- 0
#df_household_roster$school_attendance <- as.numeric(df_household_roster$school_attendance)
df_household_roster$school_attendance  <- labelled::labelled(df_household_roster$school_attendance,
  labels = c( 'Hay necesidad' = 0, 'No hay necesidad/vulnerabilidad' =1 ),
  label = "Asistencia a la escuela P205 < 18 & no asiste")   

deprivation_profile$school_attendance <- df_household_roster |>
  define_deprivation(
    .indicator = school_attendance,
    .cutoff = P205_A < 18 & P205_A > 2,
    .collapse = TRUE
  )
...
df_household_roster<- df_household_roster  |>
  group_by(uuid)  |>
  mutate(year_schooling_vf = sum_year_schooling / n())  |>
  mutate(school_attendance_vf = sum_school_attendance / n())
...
## Deprivation matrix (first 6 observations)
