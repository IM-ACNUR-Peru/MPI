...
```{r  df_household preprocessing ENPOVE 2022}
##Preprocessing
...
df_household$drinking_water  <- labelled::labelled(df_household$drinking_water,
  ...
  label = "INDICADOR 1: DRINKING WATER")
...
df_household$ind04_02v3  <- labelled::labelled(df_household$ind04_02v3,
  labels = c( 'Hay necesidad' = 0, 'No hay necesidad/vulnerabilidad' =1 ),
  label = "INDICADOR 2: ADEQUATE WALLS v3")
...
df_household$ind04_03v2  <- labelled::labelled(df_household$ind04_03v2,
  labels = c( 'Hay necesidad' = 0, 'No hay necesidad/vulnerabilidad' =1 ),
  label = "INDICADOR 3: ADEQUATE ROOFS v2")
...
df_household$housing  <- labelled::labelled(df_household$housing,
  labels = c( 'Hay necesidad' = 0, 'No hay necesidad/vulnerabilidad' =1 ),
  label = "INDICADOR 4: HOUSING CONDITIONS (household has inadequate housing materials in any of the three components: floor, roof, or walls)")
...
df_household$ind04_04  <- labelled::labelled(df_household$ind04_04,
  labels = c( 'Hay necesidad' = 0, 'No hay necesidad/vulnerabilidad' =1 ),
  label = "INDICADOR 5: OVERCROWDING ")
...
df_household$ind04_04v2  <- labelled::labelled(df_household$ind04_04v2,
  labels = c( 'Hay necesidad' = 0, 'No hay necesidad/vulnerabilidad' =1 ),
  label = "INDICADOR 5: OVERCROWDING v2")
...
df_household$cooking_fuel  <- labelled::labelled(df_household$cooking_fuel,
  labels = c( 'Hay necesidad' = 0, 'No hay necesidad/vulnerabilidad' =1 ),
  label = "INDICADOR 6: COOKING FUEL")
...
df_household$assets  <- labelled::labelled(df_household$assets,
  labels = c( 'Hay necesidad' = 0, 'No hay necesidad/vulnerabilidad' =1 ),
  label = "INDICADOR 7: ASSETS (Household does not owns 3 or more items)") 
...
df_household$assetsv2  <- labelled::labelled(df_household$assetsv2,
  labels = c( 'Hay necesidad' = 0, 'No hay necesidad/vulnerabilidad' =1 ),
  label = "INDICADOR 7: ASSETS v2 (Household does not owns 2 or more items)") 
...
df_household$ind04_07  <- labelled::labelled(df_household$ind04_07,
  labels = c( 'Hay necesidad' = 0, 'No hay necesidad/vulnerabilidad' =1 ),
  label = "INDICADOR 8: TIPO DE VIVIENDA") 
...
df_household_roster$nutrition  <- labelled::labelled(df_household_roster$nutrition,
  labels = c( 'Hay necesidad' = 0, 'No hay necesidad/vulnerabilidad' =1 ),
  label = "INDICADOR 9: Aseguramiento de salud")
...
df_household_roster$child_mortality  <- labelled::labelled(df_household_roster$child_mortality,
  labels = c( 'Hay necesidad' = 0, 'No hay necesidad/vulnerabilidad' =1 ),
  label = "INDICADOR 10: Respuesta ante enfermedad ")
...
##PENDING:
#Step 1: probablemente hay que filtrar por P207 == 1 y hace un group by CODIGO_HOGAR y quedarme con los máximos. Necesito aquí el N de miembros del hogar (nmiembrosrecod) + datos sobre las habitaciones (P105C) y dormitorios  (p106uso)
...
## Tipo de Pared## 
#improved wall: cement,stone,bricks,cement blocks, covered adobe, wood planks, as in UNHCR RMS indicator DWE04. 
#Se utilizó los hogares que tengan paredes de: 4) Tapia; 5) Quincha (caña con barro); 6) Piedra con barro; 7) Madera (pona, tornillo, etc.); 8) Estera/triplay/calamina y 9) Otro material
...
## VERSION 2
#Se utilizó los hogares que tengan paredes de: 3) Adobe; 4) Tapia; 5) Quincha (caña con barro); 6) Piedra con barro; 7) Madera (pona, tornillo, etc.); 8) Estera/triplay/calamina y 9) Otro material
...
## VERSION 3
#Se utilizó los hogares que tengan paredes de: 2) Piedra, sillar con cal o cemento; 3) Adobe; 4) Tapia; 5) Quincha (caña con barro); 6) Piedra con barro; 7) Madera (pona, tornillo, etc.); 8) Estera/triplay/calamina y 9) Otro material
...
## a partir de aqui guardo ad hoc objects por encuesta 
...
# ----------------------------------
# Compute the MPI
mpi_result_enpove2022 <- df_household |>
